...
class InHouseInstrument(Instrument):

    INSTANTIABLE = False

    def __init__(self, config, **kwargs):
        super(InHouseInstrument, self).__init__(config, **kwargs)
...
class OmegaTRHP(InHouseInstrument):

    INSTANTIABLE = True

    def __init__(self, config, **kwargs):
        super(OmegaTRHP, self).__init__(config, **kwargs)

        self.name = 'OmegaTRHP'
        self.type = 'TandRH'
        self.model = 'OmegaTRHP'
        self.tag_list = [
            'temperature',
            'relative_humidity',
            'rh',
    ...
    def get_definition_instance(self):
        ...
        return OmegaTRHP.get_definition()

    def get_definition():
        # TODO: come up with static definition method
        definition = dict()
        definition['module'] = OmegaTRHP.__module__
        definition['name'] = OmegaTRHP.__name__
        definition['mfg'] = 'InHouse'
        definition['model'] = 'OmegaTRHP'
        ...
        time_series1d['default_y_data'] = ['relative_humidity']
        source_map = {
            'default': {
                'y_data': {
                    'default': y_data
                },
                'default_y_data': ['relative_humidity']
            },
        }
