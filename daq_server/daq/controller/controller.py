...
class DummyController(Controller):
    ...
    def build_controller_meta(self):
            ...
            for inst in self.component_map["INSTRUMENTS"]["DUMMY"]["LIST"]:
                ...
                for mtype, meas in inst_meas.items():
                    ...
                    if "concentration" in meas:
                        ...
                        ts1d_meas["primary"]["concentration"] = meas["concentration"]

                        geo_z_data.append("concentration")
                        geo_meas["primary"]["concentration"] = meas["concentration"]
        ...
        if (
            len(self.component_map["INSTRUMENTS"]["GPS"]["LIST"]) > 0
            and len(self.component_map["INSTRUMENTS"]["DUMMY"]["LIST"]) > 0
        ):
            # configure GPS measurements
            ctr_meas = dict()
    ...
    def calculate_data(self, msg):
        ...
        if len(self.component_map["INSTRUMENTS"]["DUMMY"]["LIST"]) > 0:
            ...
            if id == dummy.get_id():
                ...
                dlogdp = math.pow(10, math.log10(dp[1] / dp[0]))
                ...
                entry = {
                    "DATA": {
                        "DATETIME": dt,
                        "MEASUREMENTS": {
                            "dndlogdp": {"VALUE": dndlogdp},
                            "inverted_diameter_um": {
                                "VALUE": dp,
                            },
                            "integral_concentration": {"VALUE": round(intN, 3)},
                        },
                    }
                }
