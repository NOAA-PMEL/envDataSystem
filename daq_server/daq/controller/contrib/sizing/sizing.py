...
class SizingSystem(Controller):
    ...
    def build_controller_meta(self):
        ...
        if self.has_aitken or self.has_accum or self.has_aps:
            ...
            ctr_meas[self.get_id()]["measurement_meta"]["primary_2d"] = primary_meas_2d
            ctr_meas[self.get_id()]["measurement_meta"]["primary"] = primary_meas
            ...
            sd_source_map[self.get_id()] = {
                "y_data": {"dmps": dmps_sd_y_data, "aps": aps_sd_y_data},
                "default_y_data": sd_default_y_data,
                "alias": self.alias,
                "measurement_meta": sd_meas,
            }
    ...
    def calculate_data(self, msg):
        ...
        if self.has_aitken and self.has_accum:
            ...
            if (
                dt in self.dmps_data
                and "aitken" in self.dmps_data[dt]
                and "accum" in self.dmps_data[dt]
            ):
                ...
                Tk = 293.15
                p = 1013.15
                num_bins = len(self.dmps_data[dt]["aitken"]["dp"]) + len(
                    self.dmps_data[dt]["accum"]["dp"]
                )
                ...
                self.data_ready[dt]["dmps"] = {
                    "dmps_dndlogdp": {"VALUE": dmps_dndlogdp},
                    "dmps_diameter_um": {"VALUE": dmps_dp},
                    "dmps_integral_concentration": {"VALUE": round(dmps_intN, 3)},
                }
        ...
        if self.has_aps:
            ...
            if id == aps.get_id():
                ...
                self.data_ready[dt]["aps"] = {
                    "aps_dndlogdp": {"VALUE": aps_dndlogdp},
                    "aps_diameter_um": {"VALUE": aps_dp},
                    "aps_integral_concentration": {"VALUE": round(aps_intN, 3)},
                }

        if (self.has_aitken and self.has_accum and self.has_aps) and (
            dt in self.data_ready
            and "dmps" in self.data_ready[dt]
            and "aps" in self.data_ready[dt]
        ):
            # return valid entry
            entry = {"DATA": {"DATETIME": dt, "MEASUREMENTS": dict()}}
