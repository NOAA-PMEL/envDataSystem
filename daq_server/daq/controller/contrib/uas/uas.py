...
class UASCloudyPayload(Controller):
    ...
    def __init__(self, config, **kwargs):
        super(UASCloudyPayload, self).__init__(config, **kwargs)
        self.name = 'UASCloudyPayload'

        self.msems = None
        self.msems_sh_trh = None

        self.pops = None
        self.pops_sh_trh = None

        self.cdp = None

        self.ambient_trh = None
        self.gps = None
    ...
    def build_controller_meta(self):
        ...
        # TODO: create VerticalPlot app
        vp_plot = {
            'app_type': 'VerticalProfilePlot',
            'source_map': dict()
        }
        vp_source_map = dict()

        self.has_gps = False
        self.has_msems = False
        self.has_pops = False
        self.has_cdp = False
            ...
            for inst in self.component_map['INSTRUMENTS']['GPS']['LIST']:
                ...
                for mtype, meas in inst_meas.items():
                    if 'latitude' in meas:
                        ...
                        vp_x_data.append('latitude')
                        vp_meas['primary']['latitude'] = meas['latitude']
                    ...
                    if 'longitude' in meas:
                        ...
                        vp_x_data.append('longitude')
                        vp_meas['primary']['longitude'] = meas['longitude']
                    ...
                    if 'altitude' in meas:
                        ...
                        vp_x_data.append('altitude')
                        vp_alt_dim = 'altitude'
                        vp_meas['primary']['altitude'] = meas['altitude']
        ...
        if len(self.component_map['INSTRUMENTS']['msems']['LIST']) > 0:
            self.has_msems = True
            # configure GPS measurements
            # TODO: how to specify primary GPS (or other) meas?
            msems = dict()
            ...
            for inst in self.component_map['INSTRUMENTS']['msems']['LIST']:
                ...
                # meas_meta[inst_id]['alias'] = inst_alias
                msems[inst_id] = dict()
                msems[inst_id]['alias'] = inst_alias
                msems[inst_id]['measurement_meta'] = dict()
                msems[inst_id]['measurement_meta']['primary'] = dict()
                mm = msems[inst_id]['measurement_meta']['primary']
                for mtype, meas in inst_meas.items():
        ...
        if len(self.component_map['INSTRUMENTS']['cdp']['LIST']) > 0:
            ...
            for inst in self.component_map['INSTRUMENTS']['cdp']['LIST']:
                ...
                # meas_meta[inst_id]['alias'] = inst_alias
                cdp[inst_id] = dict()
                cdp[inst_id]['alias'] = inst_alias
                cdp[inst_id]['measurement_meta'] = dict()
        ...
        if len(self.component_map['INSTRUMENTS']['trh']['LIST']) > 0:
            # self.has_trh = True
            # configure GPS measurements
            # TODO: how to specify primary GPS (or other) meas?
            trh = dict()
            ...
            for inst in self.component_map['INSTRUMENTS']['trh']['LIST']:
                ...
                # meas_meta[inst_id]['alias'] = inst_alias
                trh[inst_id] = dict()
                trh[inst_id]['alias'] = inst_alias
                trh[inst_id]['measurement_meta'] = dict()
        ...
        # plot_name = prefix + '_vert_prof'
        # self.plot_config['plots'][plot_name] = vp_plot
        # app_name = '/controller_' + self.alias['name'] + '_' + plot_name
        # self.plot_config['plots'][plot_name]['app_name'] = app_name
        # self.plot_list.append(app_name)
    ...
    def get_definition():
        definition = dict()
        definition['module'] = UASCloudyPayload.__module__
        definition['name'] = UASCloudyPayload.__name__
        definition['tags'] = [
            'size distribution',
            'particle',
            'aerosol',
