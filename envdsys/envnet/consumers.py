...
class DAQRegistryConsumer(AsyncWebsocketConsumer):
    async def connect(self):

        print(f'DAQRegistryConsumer:scope: {self.scope}')
        self.hostname = self.scope["server"][0]
        self.port = self.scope["server"][1]
        # print(f'hostname:port : {self.hostname}:{self.port}')
        # self.daqserver_group_name = 'daq_messages'
        self.manage_group_name = "envnet-manage"
        self.reg_win_group_name = "daq_registry"
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            print(f"DAQRegistryConsumer error {e}")
            return
        ...
        if message["SUBJECT"] == "PING":
            ...
            print(f'DAQRegistryConsumer: PING {message["BODY"]}')
        elif message["SUBJECT"] == "REGISTRATION":
            print(f'DAQRegistryConsumer: REGISTRATION {message["BODY"]}')
    ...
    # Receive message from room group
    async def daqreg_message(self, event):
        message = event["message"]
        # print(f'data_message: {json.dumps(message)}')
        # Send message to WebSocket
        await self.send(text_data=json.dumps({"message": message}))
